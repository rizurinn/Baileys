 ...
 */
export const isLottieBuffer = (buffer) => {
    if (buffer.length < 2)
        return false;
    let jsonBuffer;
 ...
 */
const convertToWebP = async (buffer, logger) => {
    ...
    if (!lib?.sharp) {
        throw new Boom('Sharp library is required to convert non-WebP images to WebP format. Install with: yarn add sharp', { statusCode: 400 });
    }
 ...
 */
const mediaToBuffer = async (media, context, options) => {
    ...
    if (Buffer.isBuffer(media)) {
        ...
        if (url.startsWith('data:')) {
            try {
                ...
                if (buffer.length > MAX_SIZE) {
                    throw new Boom(`${context} data URL size (${(buffer.length / 1024).toFixed(2)}KB) exceeds ${MAX_SIZE / 1024}KB limit`, { statusCode: 413 });
                }
                return buffer;
            }
            catch (error) {
        ...
        try {
            ...
            if (contentLength && parseInt(contentLength) > MAX_SIZE) {
                throw new Boom(`${context} URL file size (${(parseInt(contentLength) / 1024).toFixed(2)}KB) exceeds ${MAX_SIZE / 1024}KB limit`, { statusCode: 413, data: { url, contentLength } });
            }
            const chunks = [];
            let totalSize = 0;
            if (!response.body) {
            ...
            for await (const chunk of response.body) {
                ...
                if (totalSize > MAX_SIZE) {
                    throw new Boom(`${context} download (${(totalSize / 1024).toFixed(2)}KB) exceeded ${MAX_SIZE / 1024}KB limit`, { statusCode: 413, data: { url } });
                }
                chunks.push(buffer);
            }
        ...
        const chunks = [];
        let totalSize = 0;
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Boom(`${context} stream timeout after ${TIMEOUT}ms`, { statusCode: 408 })), TIMEOUT));
        try {
            await Promise.race([
                (async () => {
                    for await (const chunk of media.stream) {
                        ...
                        totalSize += buffer.length;
                        if (totalSize > MAX_SIZE) {
                            throw new Boom(`${context} stream size (${(totalSize / 1024).toFixed(2)}KB) exceeds ${MAX_SIZE / 1024}KB limit`, { statusCode: 413 });
                        }
                        chunks.push(buffer);
                    }
                })(),
...
export const prepareStickerPackMessage = async (stickerPack, options) => {
    ...
    // Helper: Upload Media
    const uploadMedia = async (buffer, mediaType, opts) => {
        let encFilePath;
        try {
    ...
    const attemptCompression = async (buffer, index) => {
        const MAX_SIZE = 1024 * 1024;
        const lib = await getImageProcessingLibrary();
        if (!lib?.sharp) {
            throw new Boom(`Sticker ${index + 1} exceeds the 1MB hard limit (${(buffer.length / 1024).toFixed(2)}KB). ` +
                `Sharp library required for auto-compression. Install with: yarn add sharp`, { statusCode: 400 });
        }
        try {
            ...
            if (error instanceof Boom)
                throw error;
            throw new Boom(`Sticker ${index + 1} exceeds 1MB and compression failed: ${error.message}`, {
                statusCode: 400
            });
        }
    };
    ...
    if (validStickers.length < 3 || validStickers.length > 30) {
        throw new Boom(`Sticker pack must contain between 3 and 30 valid stickers per WhatsApp official spec. ` +
            `Provided: ${validStickers.length} valid stickers ` +
            `(${stickers.length} total, ${stickers.length - validStickers.length} invalid/undefined)`, { statusCode: 400 });
    }
    if (name.length > 128) {
    ...
    const processedStickers = await Promise.all(stickers.map(async (sticker, i) => {
        if (!sticker)
            return null;
        try {
            ...
            if (sticker.isLottie !== undefined && sticker.isLottie !== isLottie) {
                throw new Boom(`Sticker ${i + 1}: explicit isLottie=${sticker.isLottie} does not match detected format (detected=${isLottie})`, { statusCode: 400 });
            }
            if (!isLottie && isWebPBuffer(webpBuffer)) {
                ...
                if (lib?.sharp) {
                    ...
                    if (metadata.width !== 512 || metadata.height !== 512) {
                        logger?.trace({ index: i, width: metadata.width, height: metadata.height }, 'Resizing sticker to 512x512 (WABA standard)');
            ...
            if (webpBuffer.length > MAX_STICKER_SIZE) {
                logger?.warn({ index: i, sizeKB: (webpBuffer.length / 1024).toFixed(2) }, `Sticker ${i + 1} exceeds 1MB, attempting compression...`);
            ...
            if (finalSizeKB > recommendedLimit) {
                logger?.warn({ index: i, sizeKB: finalSizeKB, recommendedLimit, isAnimated }, `Sticker ${i + 1} exceeds WhatsApp recommended size (${recommendedLimit}KB). ` +
            ...
            logger?.trace({ index: i, fileName, sizeKB: finalSizeKB.toFixed(2), isAnimated, isLottie }, 'Sticker processed successfully');
            return {
                fileName,
                webpBuffer,
                isAnimated,
    ...
    if (duplicateCount > 0) {
        logger?.info({ duplicateCount, uniqueStickers: stickerMetadata.length }, `Removed ${duplicateCount} duplicate stickers via deduplication`);
    }
    ...
    try {
        ...
        if (zipBuffer.length > MAX_PACK_SIZE) {
            throw new Boom(`Total pack size exceeds ${MAX_PACK_SIZE / 1024 / 1024}MB limit. ` +
                `Current size: ${(zipBuffer.length / 1024 / 1024).toFixed(2)}MB. ` +
                `Try compressing stickers or reducing pack size.`, { statusCode: 400 });
        }
    }
    catch (error) {
